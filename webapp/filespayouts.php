<?php
// ============================================
// FilesPayouts API - Dosya Listesi
...
// FilesPayouts API Ayarları
define('FILEQ_API_KEY', '862pmfvp1b6a7oenzf5');
define('FILEQ_API_URL', 'https://filespayouts.com/api/file/list');
...
function requestFileQJson($url, $cacheKey, $timeout = 30) {
    $host = parse_url($url, PHP_URL_HOST) ?: 'filespayouts.com';
    $attempts = [
...
/**
 * FilesPayouts API'den dosya listesini çeker
 * 
...
function getFileQStats() {
    $url = 'https://filespayouts.com/api/account/info?key=' . FILEQ_API_KEY;
...
function emitFileQFailure($format, $message, $errorData = null) {
    ...
    if ($format === 'html') {
        ...
        echo "<!doctype html><html lang=\"tr\"><head><meta charset=\"utf-8\"><title>FilesPayouts Hatası</title><style>body{font-family:Segoe UI,Arial,sans-serif;background:#111;color:#eee;padding:32px} .card{max-width:900px;margin:0 auto;background:#1d1d1d;border:1px solid #333;border-radius:16px;padding:24px} h1{color:#ff4d5f} code{display:block;margin-top:16px;padding:12px;background:#0f0f0f;border-radius:10px;color:#ffd0d5;white-space:pre-wrap;word-break:break-word}</style></head><body><div class=\"card\"><h1>FilesPayouts verisine erişilemedi</h1><p>{$safeMessage}</p><code>{$safeDiagnostic}</code></div></body></html>";
        return;
    }
...
try {
    ...
    $processedFiles = array_map(function($file) {
        return [
            ...
            'link' => $file['link'],
            'download_link' => "https://filespayouts.com/{$file['file_code']}.html",
            'size' => $file['size'],
    ...
    // HTML formatında döndür (debug için)
        ...
        <head>
            ...
            <title>FilesPayouts Dosyalarım - BELGESELSEMOFLIX</title>
        ...
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <p class="subtitle">FilesPayouts Dosya Yönetimi</p>
                </div>
